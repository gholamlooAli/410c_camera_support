	...
	void main()
	{
		...
		if (0.0 == mod(xcoord , 2.0)) {	// even
			if (0.0 == mod(xcoord , 4.0)) {	// even
				yuv.x = texture2D(s_luma_texture, v_tex_coord).x;
			}
			else{
				yuv.x = texture2D(s_luma_texture, v_tex_coord).z;
			}
			//yuv.y = texture2D(s_chroma_texture, v_tex_coord).x;//u
			//yuv.z = texture2D(s_chroma_texture,vec2(v_tex_coord.x + texel_width, v_tex_coord.y)).r;			//v
		}	
		else {// odd
			yuv.x = texture2D(s_luma_texture, v_tex_coord).y;
			//yuv.z = texture2D(s_chroma_texture, v_tex_coord).x;//u
			//yuv.y = texture2D(s_chroma_texture,vec2(v_tex_coord.x - texel_width, v_tex_coord.y)).r;			//v
		}*/
